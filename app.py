...
# Inicializa DB (cria tabela/índices se necessário) — agora também cria as tabelas de catálogos e cirurgias
init_db()
...
# ======================== NOVAS ABAS: Cadastro & Cirurgias ========================
st.divider()
st.header("Gestão de Cirurgias")

tabs = st.tabs(["📚 Cadastro (Tipos & Situações)", "🩺 Cirurgias"])

# --------- Aba 1: Cadastro de Tipos de Procedimento e Situações -----------
with tabs[0]:
    ...
    st.subheader("Situações da Cirurgia")
...
with tabs[1]:
    ...
    with st.form("form_cirurgia"):
        ...
        data_cirurgia = st.text_input("Data da cirurgia (ex.: 11/01/2026)", value=(base["Data"] if base is not None else ""))
    ...
    st.subheader("Lista de Cirurgias")
